...
export const Navigation = () => {
  ...
  const [togglePath, setTogglePath] = useState(`/${toggleLocale}`);

  useEffect(() => {
    if (typeof window !== 'undefined') {
      const path = window.location.pathname || '/';
      const newPath = path.replace(/^\/(en|zh)/, `/${toggleLocale}`);
      setTogglePath(newPath);
    }
  }, [toggleLocale]);
      ...
      <div className="max-w-7xl mx-auto px-6 py-4">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            ...
            >
              <svg className="w-5 h-5 text-chinese-ink-600" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
